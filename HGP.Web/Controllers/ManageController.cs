...
namespace HGP.Web.Controllers
{
    ...
    {
        private static readonly ILogger Logger = Log4NetLogger.GetLogger();
        public ManageController()
        {
        }

        public ManageController(PortalUserService userService)
        {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            var model = IoC.Container.GetInstance<ModelFactory>().GetModel<IndexViewModel>();

            model.HasPassword = HasPassword();
            model.PhoneNumber = await UserService.GetPhoneNumberAsync(User.Identity.GetUserId());
            model.TwoFactor = await UserService.GetTwoFactorEnabledAsync(User.Identity.GetUserId());
            model.Logins = await UserService.GetLoginsAsync(User.Identity.GetUserId());
                model.BrowserRemembered = await AuthenticationManager.TwoFactorBrowserRememberedAsync(User.Identity.GetUserId());
...
#region Helpers
        ...
        private async Task SignInAsync(PortalUser user, bool isPersistent)
        {
